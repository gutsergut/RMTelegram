...
class CatalogService
{
    ...
    public function listMetas(int $page = 1, int $limit = 5, array $filters = []): array
    {
        ...
        if ($hasSearchFilter) {
            try {
                ...
                $len = mb_strlen($searchQueryLower, 'UTF-8');
                for ($i = 0; $i < $len; $i++) {
                    $char = mb_substr($searchQueryLower, $i, 1, 'UTF-8');
                    $ord = mb_ord($char, 'UTF-8');
                    // Экранируем не-ASCII символы (кириллица и др.)
                    if ($ord > 127) {
                        $searchUnicode .= sprintf('\\\\u%04x', $ord);
                    } else {
                        $searchUnicode .= $char;
                    }
                }
                $searchUnicodeLike = '%' . $db->escape($searchUnicode, true) . '%';

                // Ищем alias'ы полей, соответствующие поисковому запросу (через text/title в options)
                // Например: поиск "Эквадор" найдёт alias "ekvador" через опцию {text:"Эквадор", value:"ekvador"}
                $matchingFieldAliases = [];
                try {
                    $qFields = $db->getQuery(true)
                        ->select($db->quoteName('options'))
                        ->from($db->quoteName('#__radicalmart_fields'))
                        ->where($db->quoteName('state') . ' = 1')
                        ->where($db->quoteName('options') . ' != ' . $db->quote(''))
                        ->where($db->quoteName('options') . ' != ' . $db->quote('[]'))
                        ->where($db->quoteName('options') . ' != ' . $db->quote('{}'));
                    $fieldsOptions = $db->setQuery($qFields)->loadColumn();

                    foreach ($fieldsOptions as $optJson) {
                        $opts = json_decode($optJson, true);
                    ...
                    $matchingFieldAliases = array_unique(array_filter($matchingFieldAliases));
                    if ($debug && !empty($matchingFieldAliases)) {
                        LogHelper::debug('listMetas: search found field aliases: ' . json_encode($matchingFieldAliases), 'radicalmart_telegram_catalog');
                    }
                } catch (\Throwable $e) {}

                // Поиск по products: title, alias, code, search_text, fields (JSON)
                // Для полей JSON добавляем поиск по Unicode-экранированной версии
                ...
                $orConditions = [
                    ...
                    $db->quoteName('p.fields') . ' LIKE ' . $db->quote($searchUnicodeLike),
                ];

                // Добавляем поиск по найденным alias'ам полей (например: "strana":"ekvador")
                foreach ($matchingFieldAliases as $fAlias) {
                    $aliasPattern = '%":"' . $db->escape($fAlias, true) . '"%';
                    $orConditions[] = $db->quoteName('p.fields') . ' LIKE ' . $db->quote($aliasPattern);
                }
    ...
    private function mapProductForMeta(object $it, ?string $rawCoordsFromDb = null): array
    {
        ...
        $extendedEnabled=(int)$params->get('cardview_enabled',0)===1; static $resolved=null; if($resolved===null){
            $resolved=['badge'=>[],'subtitle'=>[],'weight'=>'','weight_id'=>0,'map_coords'=>'','map_coords_id'=>0,'map_country'=>'','map_country_id'=>0,'map_country_options'=>[]];
            ...
            $mapCountryRaw=$params->get('map_country_field',''); if(is_array($mapCountryRaw)) $mapCountryRaw=reset($mapCountryRaw);
            ...
            $idToAlias=[]; $idToOptions=[]; if($idsNeeded){ try { $db=Factory::getContainer()->get('DatabaseDriver'); $q=$db->getQuery(true)->select([$db->quoteName('id'),$db->quoteName('alias'),$db->quoteName('options')])->from($db->quoteName('#__radicalmart_fields'))->where($db->quoteName('id').' IN ('.implode(',',array_map('intval',$idsNeeded)).')'); $rows=$db->setQuery($q)->loadAssocList(); foreach($rows as $r){ $idToAlias[(int)$r['id']]=trim((string)$r['alias']); if(!empty($r['options'])){ $opts=json_decode($r['options'],true); if(is_array($opts)) $idToOptions[(int)$r['id']]=$opts; } } } catch(\Throwable $e){} }
            ...
            if($mapCountryRaw!==''){
                if(ctype_digit((string)$mapCountryRaw)){
                    ...
                    // Сохраняем опции поля страны для получения флагов и display-текстов
                    if(isset($idToOptions[$id])) $resolved['map_country_options']=$idToOptions[$id];
                } else {
        ...
        // RadicalMart модель может вернуть либо alias (kolumbiya), либо display text (Колумбия)
        // Опции поля ключевываются по alias, но нужно найти image
        $mapCountryVal='';
        $mapCountryAliasVal='';
        $mapCountryImage='';
        $countryOptions = $resolved['map_country_options'] ?? [];
        if($mapCountryAlias!==''){
            // Получаем значение страны из fields товара (может быть alias или display text)
            $countryFieldValue = '';
            if(isset($fields[$mapCountryAlias])){
                $mcof=$fields[$mapCountryAlias];
                if(is_string($mcof)){
                    $countryFieldValue = $mcof;
                } else {
                    // Сначала пробуем extractFieldDisplay (вернёт display text если есть)
                    $countryFieldValue=$this->extractFieldDisplay($mcof);
                    if($countryFieldValue==='') $countryFieldValue=$this->extractFieldValue($mcof);
                }
            }
            if($countryFieldValue==='' && $mapCountryId>0 && isset($fields[$mapCountryId])){
            ...
            // Ищем в опциях поля соответствующую запись
            // Пробуем сначала по ключу (alias), затем по значению text
            $foundOpt = null;
            $foundAlias = '';
            if($countryFieldValue!=='' && !empty($countryOptions)){
                // 1) Прямой поиск по ключу (если countryFieldValue — это alias)
                if(isset($countryOptions[$countryFieldValue])){
                    $foundOpt = $countryOptions[$countryFieldValue];
                    $foundAlias = $countryFieldValue;
                } else {
                    // 2) Поиск по text/label (если countryFieldValue — это display text)
                    foreach($countryOptions as $alias => $opt){
                        if(!is_array($opt)) continue;
                        $optText = (string)($opt['text'] ?? $opt['label'] ?? $opt['title'] ?? '');
                        if($optText !== '' && mb_strtolower($optText, 'UTF-8') === mb_strtolower($countryFieldValue, 'UTF-8')){
                            $foundOpt = $opt;
                            $foundAlias = is_string($alias) ? $alias : (string)($opt['value'] ?? '');
                            break;
                        }
                    }
                }
            ...
            // Извлекаем данные из найденной опции
            if($foundOpt !== null && is_array($foundOpt)){
                $mapCountryVal = (string)($foundOpt['text'] ?? $foundOpt['label'] ?? $foundOpt['title'] ?? $countryFieldValue);
                $mapCountryAliasVal = $foundAlias !== '' ? $foundAlias : (string)($foundOpt['value'] ?? $countryFieldValue);
                // Извлекаем путь к изображению флага
                $imgRaw = (string)($foundOpt['image'] ?? '');
                if($imgRaw!==''){
                    // Убираем JImage anchor (#joomlaImage://...) если есть
                    $mapCountryImage = preg_replace('/#joomlaImage:.*$/', '', $imgRaw);
                    // Убираем обратные слеши (экранирование из JSON)
                    $mapCountryImage = str_replace('\\/', '/', $mapCountryImage);
                    // Убираем ведущий слеш если есть (фронтенд сам добавит /)
                    $mapCountryImage = ltrim($mapCountryImage, '/');
                }
            } else {
                // Fallback: используем полученное значение как display text
                $mapCountryVal = $countryFieldValue;
                $mapCountryAliasVal = $countryFieldValue;
            }
        }
        $out['map_country']=$mapCountryVal;
        $out['map_country_alias']=$mapCountryAliasVal;
        $out['map_country_image']=$mapCountryImage;
