...
class ProfileService
{
    public function getProfile(int $chatId): array
    {
        ...
        if ($userId > 0) {
            ...
            // Get contacts from RadicalMart users table (may not exist on some installations)
            try {
                ...
                // Table may not exist on some RadicalMart versions
            }
    ...
    public function createReferralCode(int $userId, string $currency = '', string $customCode = ''): ?string
    {
        try {
            ...
            LogHelper::error('ProfileService::createReferralCode error: ' . $e->getMessage());
            return null;
        }
    }
