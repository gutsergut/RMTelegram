...
<head>
    ...
    <script>
        ...
        function renderSummary(cart) {
            ...
            // Don't show "0 ₽" discount or negative discount (which happens when shipping is included in final)
            // Only show if there's a real positive discount and it's not from promo codes
            const discountVal = parseNumber(cart.total.discount);
            const discountFromString = parseNumber(cart.total.discount_string);
            // Ensure discount is positive both as number and in string representation
            if (cart.total.discount_string && discountVal > 0 && discountFromString > 0 && !cart.plugins?.bonuses?.codes_discount_string) {
    ...
    <div id="checkout-form" hidden>
        <form id="checkout-form-el" onsubmit="event.preventDefault(); submitOrder();">
            ...
            <div class="checkout-section">
                ...
                <div class="uk-margin-small">
                    <input class="uk-input" type="text" name="second_name" placeholder="<?php echo Text::_('COM_RADICALMART_TELEGRAM_SECOND_NAME'); ?>">
                </div>
                <div class="uk-margin-small">
