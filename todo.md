...
- [x] Сервис‑слой: Telegram API client (TelegramClient), построитель сообщений/клавиатур, обработчик апдейтов (UpdateHandler).
...
- [x] Валидация `secret`.
- [x] Базовый обработчик апдейтов и клиент Telegram (UpdateHandler): `/start`/`/help`.
- [x] Логирование, защита от повторов (идемпотентность по update_id).
- [x] Команды: `Каталог`, `Корзина`, `Оформить заказ`, `Мои заказы`, `Промокод`, `Баллы`.
- [x] Состояния диалога: idle → browsing → product → cart → checkout → payment (SessionStore + FSM).

Критерии приемки:
- Бот отвечает на команды, переключает состояния, есть кнопки/клавиатуры.

М4. Каталог и поиск (1 дн)
- [x] Дерево категорий, пагинация, быстрый поиск (название/артикул) в WebApp + чат.
- [x] Карточка товара: фото, цена(ы), наличие, опции/вариации (вес).
- [x] Кнопки: Добавить в корзину, Поделиться (через WebApp).

Критерии приемки:
- Навигация по категориям и просмотр товаров без ошибок.

М5. Корзина и оформление (1–2 дн)
- [x] Хранение корзины в `sessions` (для гостя) и привязка к `user_id` при авторизации (CartService).
- [x] Промокоды/баллы: ввод/применение, валидация, пересчёт сумм через модели RadicalMart (BonusesService).
- [x] Контакты: запрос телефона (через Telegram contact), ФИО, email.

Критерии приемки:
- Добавление/удаление/изменение позиций, применение промокода и/или баллов, корректные суммы.

М6. Доставка (ApiShip) (1 дн)
- [x] Запрос адреса: геолокация/ручной ввод → нормализация (DaData если доступно).
- [x] Получение тарифов через ApiShipIntegrationHelper и выбор варианта.
- [x] Поддержка ПВЗ: выдача списка по bbox, выбор ПВЗ (PvzService, api.pvz, api.setpvz).

Критерии приемки:
- Для заданного адреса возвращаются тарифы, выбор сохраняется в сессии.

М7. Создание заказа и оплата (0.5–1 дн)
- [x] Сбор `jform` для `CheckoutModel::createOrder`, привязка корзины, автора (`created_by`) — CheckoutService::createOrder().
- [x] Возврат номера заказа и ссылки на оплату (`payment.pay&order_number=...`).
- [x] Обработка статусов оплаты (webhook pre_checkout_query/successful_payment → уведомление в Telegram).
...
- [x] Механизм привязки: одноразовый код на сайте → отправка боту → сохранение `chat_id`↔`user_id` (TelegramUserHelper).
- [x] Уведомления о статусах заказа: через `plg_radicalmart_message_telegram` + прямые сообщения из компонента.

Критерии приемки:
- Пользователь получает личные уведомления по смене статуса.

М9. Админка (1 дн)
- [x] Управление пользователями бота (поиск/развязка) — view=links.
- [x] История платежей, возвраты — view=payments.
...
- [x] Rate‑limit по chat_id (DB rate-limit per chat/tg_user во всех API).
- [x] Защита от повторной обработки апдейтов (идемпотентность по update_id).
- [x] Централизованный лог (Joomla Log com_radicalmart.telegram) + debug‑флаги.
- [x] Фолбэки ошибок для пользователя (JsonResponse с error).
- [x] Nonce для мутаций (guardNonce в ApiController).
- [x] HMAC проверка initData (guardInitData).

М11. Тестирование и документация (0.5 дн)
- [ ] Smoke‑тесты сценариев: каталог → корзина → оформление → оплата.
- [x] Документация по установке и эксплуатации — docs/README.md обновлён (v5.0.5).
...
- [x] Получить `provider_token` для YooKassa — **ГОТОВО** (390540012:LIVE:84332). **Robokassa — не будем делать.**
- [x] Сформировать Invoice и payload (order_number), обработать `pre_checkout_query`/`successful_payment` (уведомление в чате + смена статуса заказа).
- [x] Telegram Stars: прайсинг (курс из RUB + % конвертации), ограничения по категориям/товарам, инвойс XTR — **ГОТОВО**.
- [x] Синхронизация статусов: при успехе оплаты в Telegram — обновляем заказ в RadicalMart и шлём персональное уведомление — **ГОТОВО**.
- [x] Настройки: включение/выключение Telegram Payments по категории (allowed/excluded) — **ГОТОВО** в плагинах Stars и Cards.
- [x] Возвраты/частичные возвраты: админка (view=payments), YooKassa Refund API — **ГОТОВО**. Stars — не поддерживается.
- [x] Плагинная архитектура оплаты: плагины `telegramcards` (YooKassa) и `telegramstars` — **ГОТОВО**.
- [ ] Для физических заказов: fallback на ссылку если провайдер недоступен (опционально).
...
### Авторизация — задачи
- [x] Поток: Telegram ID + запрос телефона (requestContact) → связывание с пользователем сайта.
- [x] Fallback: SMS/звонок через `com_j_sms_registration` (sms.ru), если телефон не отправлен.
- [x] Маппинг `chat_id`↔`user_id` и автоматический логин при совпадении телефона.
- [x] Политика повторных привязок и разлинковки в админке (view=links).
...
- [x] Трекинг незавершённых корзин в сессиях бота.
- [x] Конфиг графиков: X минут/часов, N повторов, тихие часы.
- [x] Opt‑in пользователя и хранение согласия.
- [x] Шаблоны сообщений и deep‑links для возврата к корзине/товарам.
- [x] Task плагин `radicalmart_telegram_cart_reminders` — автоматическая рассылка.

### Онлайн‑режим — задачи
- [x] Restock: планировщик раз в день + ручной запуск из админки; выборка товаров «в наличии» и рассылка.
- [x] Restock для мета‑товаров: отправлять ссылку на вариант с минимальным весом.
- [x] Task плагин `radicalmart_telegram_restock` + API + кнопка «Сообщить о поступлении» в WebApp.
- [ ] Трекер доставки: интеграция со службами (Яндекс Доставка, СДЭК, 5post) по номеру отправления — **ОТЛОЖЕНО по решению**.
...
### WebApp — задачи
- [x] Настроить разрешённые домены у BotFather (включая `https://cacao.land`).
- [x] Бутстрап WebApp: проверка `initData`, верификация хеш‑подписи (guardInitData в ApiController).
- [x] UI: каталог/карточка/корзина/оформление, карта ПВЗ, платежные экраны.
- [x] Шина событий: синхронизация состояния между чат‑диалогом и WebApp (через API + SessionStore).
- [x] Режим «минимальный шаблон» для WebApp‑страниц (tmpl=tgwebapp).
...
- [x] FSM состояний: сценарии browsing → product → cart → checkout реализованы; основной flow через WebApp.
...
- [x] Пагинация/подгрузка более ранних заказов; фильтры по статусу (OrderService::getOrders с page/limit/status).
...
- [x] **WebApp**: проверка согласий при первом открытии, модальное окно с ссылками на политики (consent-overlay + ensureConsents())
- [x] **Интеграция AcyMailing**: синхронизация consent_marketing с подпиской/отпиской в AcyMailing
  - [x] При принятии согласия на рассылку → добавить в лист AcyMailing
  - [x] При отзыве → отписать из листа
  - [x] Настройка ID листа AcyMailing в конфиге компонента (config.xml + AcyMailingListField)
  - [ ] Двусторонняя синхронизация: webhook AcyMailing → обновление таблицы бота (будущее)
...
- [x] **Команды управления подписками**:
  - [x] `/settings` → показ текущих согласий + inline-кнопки вкл/выкл
  - [x] Отзыв всех согласий (с предупреждением и подтверждением)
  - [x] Переключение рассылки и напоминаний о корзине
- [x] **Языковые константы**: добавлены тексты для управления подписками (ru-RU)
...
- [x] Возвраты/частичные возвраты (Cards) — **ГОТОВО**:
 - [x] Admin‑инструмент (view=payments) для отметки возврата/частичного возврата (смена статуса и лог).
 - [x] Валидация суммы (<= итога заказа) и показ результата провайдера.
 - [x] Refund API: Telegram Cards — вызов YooKassa /v3/refunds (по provider_payment_charge_id).
...
### 2025-05-23
- [x] **Корзина: товары возвращались после удаления** — причина: `CartService::bindUserCart()` вызывал `setCookies()`, но не устанавливал state модели → создавались дубликаты корзин. Исправлено: добавлены `setState('cart.id')` и `setState('cart.code')` после `setCookies()`. Добавлены endpoints `api.debugCarts` и `api.cleanupCarts` для диагностики.
- [x] **BackButton возвращал на страницу согласий** — причина: при переходе по bottom-nav терялся параметр `chat` в URL. Исправлено: функция `api()` в шаблонах теперь использует `window.TG_CHAT_ID` как fallback.
