...
class ApiShipIntegrationHelper
{
    public static function calculateTariff(int $shippingId, object $cart, string $pointId, string $provider): array
    {
        ...
        LogHelper::debug("$logPrefix START", 'com_radicalmart.telegram.tariff');
        ...
        try {
            if (!class_exists(ApiShip::class)) {
                LogHelper::warning("$logPrefix ApiShip class not found", 'com_radicalmart.telegram.tariff');
            ...
            if (empty($token)) {
                LogHelper::warning("$logPrefix Token is empty", 'com_radicalmart.telegram.tariff');
                $debugInfo['error'] = 'Token is empty';
                return ['__debug' => $debugInfo];
            }
            LogHelper::debug("$logPrefix Token found (len=" . strlen($token) . ")", 'com_radicalmart.telegram.tariff');
            $debugInfo['token_len'] = strlen($token);

            $senders = $params->get('sender', []);
            $senderKeys = array_keys((array)$senders);
            LogHelper::debug("$logPrefix Senders config keys: " . implode(', ', $senderKeys), 'com_radicalmart.telegram.tariff');
            $debugInfo['sender_keys'] = $senderKeys;

            $senderParams = $senders[$provider] ?? null;
            if (!$senderParams) {
                LogHelper::warning("$logPrefix Sender params for provider '$provider' NOT FOUND. Available: " . json_encode($senderKeys), 'com_radicalmart.telegram.tariff');
                $debugInfo['error'] = "Sender params for provider '$provider' NOT FOUND";
                $debugInfo['available_providers'] = $senderKeys;
                return ['__debug' => $debugInfo];
            }
            LogHelper::debug("$logPrefix Sender params found: " . json_encode($senderParams), 'com_radicalmart.telegram.tariff');
            $debugInfo['sender_params'] = $senderParams;

            $pickupType = (int)($senderParams['pickup_type'] ?? 1);
            LogHelper::debug("$logPrefix pickupType=$pickupType", 'com_radicalmart.telegram.tariff');
            ...
            if ($pickupType === 2) {
                ...
                LogHelper::debug("$logPrefix pointInId=" . $requestData['pointInId'], 'com_radicalmart.telegram.tariff');
            } else {
                $requestData['from'] = [
                    'addressString' => $senderParams['address'] ?? '',
                    'countryCode'   => $senderParams['country'] ?? 'RU',
                ];
                LogHelper::debug("$logPrefix from=" . json_encode($requestData['from']), 'com_radicalmart.telegram.tariff');
            }
            ...
            LogHelper::debug("$logPrefix Products: $productCount, places count: " . count($requestData['places']) . ", codCost: " . $requestData['codCost'], 'com_radicalmart.telegram.tariff');
            ...
            // Call Calculator
            LogHelper::debug("$logPrefix Calling ApiShipHelper::calculator with requestData=" . json_encode($requestData, JSON_UNESCAPED_UNICODE), 'com_radicalmart.telegram.tariff');

            $result = ApiShipHelper::calculator($token, $requestData, 'telegram.tariff.calc');

            // Log FULL response for debugging
            $fullResponse = $result->toArray();
            LogHelper::debug("$logPrefix FULL API response: " . json_encode($fullResponse, JSON_UNESCAPED_UNICODE), 'com_radicalmart.telegram.tariff');
            $debugInfo['full_response'] = $fullResponse;

            LogHelper::debug("$logPrefix Calculator response keys: " . implode(', ', array_keys($fullResponse)), 'com_radicalmart.telegram.tariff');
            $debugInfo['response_keys'] = array_keys($fullResponse);

            // Parse result
            $deliveryToPoint = $result->get('deliveryToPoint', []);
            LogHelper::debug("$logPrefix deliveryToPoint raw: " . json_encode($deliveryToPoint), 'com_radicalmart.telegram.tariff');
            ...
            // Debug: log FULL content of each deliveryToPoint element
            if (!empty($deliveryToPoint)) {
                foreach ($deliveryToPoint as $idx => $dtp) {
                    LogHelper::debug("$logPrefix deliveryToPoint[$idx] FULL: " . json_encode($dtp, JSON_UNESCAPED_UNICODE), 'com_radicalmart.telegram.tariff');
                    $debugInfo["delivery_to_point_$idx"] = $dtp;
                }
            }
            ...
            if (!empty($deliveryToPoint)) {
                foreach ($deliveryToPoint as $providerResult) {
                    ...
                    LogHelper::debug("$logPrefix Checking providerKey=$pk vs requested=$provider", 'com_radicalmart.telegram.tariff');
                ...
                if (empty($tariffs) && !empty($deliveryToPoint[0]->tariffs)) {
                    ...
                    LogHelper::debug("$logPrefix Using fallback tariffs from first provider", 'com_radicalmart.telegram.tariff');
                }
            }
            LogHelper::debug("$logPrefix Tariffs count: " . count($tariffs), 'com_radicalmart.telegram.tariff');
            ...
            if (!empty($tariffs) && !empty($senderParams['tariffs_regexp'])) {
                ...
                LogHelper::debug("$logPrefix Tariffs filtered by regexp: $beforeCount -> " . count($tariffs), 'com_radicalmart.telegram.tariff');
                $debugInfo['tariffs_after_filter'] = count($tariffs);
            }

            LogHelper::debug("$logPrefix END - returning " . count($tariffs) . " tariffs", 'com_radicalmart.telegram.tariff');
            ...
            LogHelper::error('ApiShip calc error: ' . $e->getMessage());
