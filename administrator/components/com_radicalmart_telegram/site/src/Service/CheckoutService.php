...
{
    /**
     * Sync checkout data from SessionStore to Joomla session
     * This is needed for RadicalMart CheckoutModel compatibility
     */
    public function syncToJoomlaSession(array $checkoutData): void
    {
        ...
        // CRITICAL: Sync plugins.bonuses from SessionStore to Joomla session
        // This is the PRIMARY source for discount calculation in RadicalMart bonuses plugin
        // Structure: {"plugins":{"bonuses":{"codes":[5],"points":"0"}}}
        if (!empty($checkoutData['plugins']['bonuses'])) {
            ...
            $sessionData['plugins']['bonuses'] = array_replace_recursive(
                $sessionData['plugins']['bonuses'],
                $checkoutData['plugins']['bonuses']
            );
            // Ensure required keys for bonuses plugin
            if (!isset($sessionData['plugins']['bonuses']['points'])) {
                $sessionData['plugins']['bonuses']['points'] = '0';
            }
            // Trigger recalculation flags expected by bonuses plugin
            $sessionData['plugins']['bonuses']['recalculate'] = 1;
            $sessionData['plugins']['bonuses']['recalculate_discounts'] = 1;
            LogHelper::debug('[CheckoutService::syncToJoomlaSession] Synced plugins.bonuses from SessionStore: ' . json_encode($checkoutData['plugins']['bonuses']));
        }

        // Sync promo code to plugins.bonuses.codes for RadicalMart/applyPromoToCart compatibility
        // RadicalMart expects plugins.bonuses.codes as array of code IDs, not code names!
        if (!empty($checkoutData['code'])) {
            ...
            // Use code_id if available, otherwise try to keep the original format
            if (!empty($checkoutData['code_id'])) {
                // RadicalMart expects array of code IDs: [5] not "funbayu"
                $sessionData['plugins']['bonuses']['codes'] = [(int) $checkoutData['code_id']];
            }
        ...
        // Also sync code_id to session for backward compatibility
        if (!empty($checkoutData['code_id'])) {
            $sessionData['code_id'] = $checkoutData['code_id'];
        }

        $app->setUserState('com_radicalmart.checkout.data', $sessionData);

        LogHelper::debug('[CheckoutService::syncToJoomlaSession] Synced: ' . json_encode(array_keys($checkoutData)));
        // Extra diagnostic snapshot of Joomla session state for checkout
        try {
            LogHelper::debug('[CheckoutService::syncToJoomlaSession] SESSION com_radicalmart.checkout.data: ' . json_encode($sessionData));
        } catch (\Throwable $e) {}
    }
     ...
     */
    public function loadAndSyncCheckoutData(int $chatId): array
    {
        ...
        // Detailed logging for debugging promo code discount issues
        $pluginsBonuses = $checkoutData['plugins']['bonuses'] ?? null;
        LogHelper::debug('[CheckoutService::loadAndSyncCheckoutData] Loaded for chat=' . $chatId . ': ' . json_encode($checkoutData) . ' | plugins.bonuses=' . json_encode($pluginsBonuses));
    ...
    public function setPvz(int $chatId, array $pvzData, int $shippingId = 0, string $tariffId = ''): array
    {
        ...
        if ($shippingId > 0 && class_exists(ApiShip::class)) {
            ...
            if ($selectedTariff) {
                ...
                $checkoutData['shipping']['price'] = ['base' => $deliveryCost];
            } else {
        ...
        return [
            ...
            'order' => $orderData,
            'pvz' => $checkoutData['shipping']['point'],
            'tariffs' => $tariffs,
    ...
    private function buildOrderData($order, $selectedTariff): ?array
    {
        ...
        // Determine if promo code or points are applied in session
        $app = \Joomla\CMS\Factory::getApplication();
        $sess = (array) $app->getUserState('com_radicalmart.checkout.data', []);
        $pluginsBonuses = (array) ($sess['plugins']['bonuses'] ?? []);
        $hasPromo = !empty($sess['code']) || !empty($pluginsBonuses['codes']);
        $hasPoints = isset($pluginsBonuses['points']) && (float)$pluginsBonuses['points'] > 0;
        ...
        // UI rule: show discount only when promo code or points are applied
        if (!($hasPromo || $hasPoints)) {
            $productDiscount = 0;
        }
