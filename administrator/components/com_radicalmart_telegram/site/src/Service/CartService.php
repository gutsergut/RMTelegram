...
class CartService
{
    ...
    protected function ensureModelState(CartModel $model, int $chatId): void
    {
        ...
        $cart = $payload['cart'] ?? [];
        if (!empty($cart['id'])) { $model->setState('cart.id', (int) $cart['id']); }
        if (!empty($cart['code'])) { $model->setState('cart.code', (string) $cart['code']); }
    }
    ...
    protected function persistCartFromResult(int $chatId, array $result): void
    {
        ...
        if ($cartObj && is_object($cartObj)) {
            [$state, $payload] = $this->store->getStatePayload($chatId);
            $payload['cart'] = [ 'id' => (int) ($cartObj->id ?? 0), 'code' => (string) ($cartObj->code ?? '') ];
    ...
    {
        if (!$cart) { return 'Корзина пуста.'; }
        $lines = []; $lines[] = 'Корзина:'; $i = 1;
        ...
        if (!empty($cart->total)) {
            ...
            if ($total !== '') { $lines[] = ''; $lines[] = 'Итого: ' . $total; }
        }
        return implode("\n", $lines);
    }
}
