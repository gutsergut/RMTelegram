...
class SessionStore
{
    ...
    public function getStatePayload(int $chatId): array
    {
        ...
        $payload = [];
        if (!empty($row['payload'])) { $json = json_decode((string)$row['payload'], true); if (is_array($json)) $payload = $json; }
        return [$state, $payload];
    }

    public function isDuplicate(int $chatId, int $updateId): bool
    { $row = $this->get($chatId); if (!$row) return false; $last = (int) ($row['last_update_id'] ?? 0); return $updateId <= $last; }

    public function setLastUpdate(int $chatId, int $updateId): void
    {
        $db = $this->db; $now = (new \Joomla\CMS\Date\Date())->toSql();
        $query = $db->getQuery(true)
        ...
        if ($db->getAffectedRows() === 0) {
            $obj = (object) [ 'chat_id' => $chatId, 'state' => 'idle', 'payload' => null, 'cart_snapshot' => null, 'expires_at' => null, 'updated' => $now, 'last_update_id' => $updateId ];
    ...
    public function setState(int $chatId, string $state, ?array $payload = null): void
    {
        ...
        $db->setQuery($query)->execute();
        if ($db->getAffectedRows() === 0) {
            $obj = (object) [ 'chat_id' => $chatId, 'state' => $state, 'payload' => $payloadJson, 'cart_snapshot' => null, 'expires_at' => null, 'updated' => $now, 'last_update_id' => 0 ];
            $db->insertObject('#__radicalmart_telegram_sessions', $obj);
        }
    }
}
