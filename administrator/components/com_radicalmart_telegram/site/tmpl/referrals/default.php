<?php
/**
 * Referrals view template
 * @version 2025-11-29-v10
 */
...
function rmt_text($key, $fallback = null) {
    $text = Text::_($key);
    // If not translated (returns same key), use fallback
...
$fallbacks = [
    'COM_RADICALMART_TELEGRAM_REFERRALS' => 'Реферальная программа',
    'COM_RADICALMART_TELEGRAM_REFERRALS_LOGIN_REQUIRED' => 'Войдите в систему для просмотра реферальной программы',
    'COM_RADICALMART_TELEGRAM_REFERRALS_NOT_IN_PROGRAM' => 'Вы ещё не участвуете в программе',
    'COM_RADICALMART_TELEGRAM_REFERRALS_NOT_IN_PROGRAM_DESC' => 'Приглашайте друзей по реферальной ссылке и получайте бонусы за их покупки!',
    'COM_RADICALMART_TELEGRAM_REFERRALS_COUNT' => 'Рефералов',
    'COM_RADICALMART_TELEGRAM_REFERRALS_EARNED_POINTS' => 'Заработано баллов',
    'COM_RADICALMART_TELEGRAM_REFERRALS_MY_CODES' => 'Мои реферальные коды',
    'COM_RADICALMART_TELEGRAM_REFERRALS_NO_CODES' => 'У вас пока нет реферальных кодов',
    'COM_RADICALMART_TELEGRAM_REFERRALS_MY_REFERRALS' => 'Мои рефералы',
    'COM_RADICALMART_TELEGRAM_REFERRALS_NO_REFERRALS' => 'У вас пока нет рефералов',
    'COM_RADICALMART_TELEGRAM_REFERRALS_INVITED_BY' => 'Кто меня пригласил',
    'COM_RADICALMART_TELEGRAM_REFERRALS_SHARE_CODE_HINT' => 'Поделитесь реферальной ссылкой с друзьями!',
];

// Custom text function with fallback
$T = function($key) use ($fallbacks) {
    $text = Text::_($key);
    if ($text === $key && isset($fallbacks[$key])) {
...
<head>
    ...
    <style>
        ...
        /* Cart badge */
        #cart-badge, .cart-badge { position: absolute; top: 2px; right: 6px; background: #f0506e; color: white; border-radius: 10px; padding: 2px 6px; font-size: 10px; font-weight: bold; min-width: 18px; text-align: center; }
...
<div id="referrals-app" class="uk-container uk-container-small uk-padding-small">
    <div class="uk-flex uk-flex-middle uk-margin-small-bottom">
        ...
        <h1 class="uk-h3 uk-margin-remove"><?php echo $T('COM_RADICALMART_TELEGRAM_REFERRALS'); ?></h1>
    </div>

    <?php if (!$this->userId): ?>
    <div class="uk-card uk-card-default uk-card-body uk-text-center">
        <span uk-icon="icon: lock; ratio: 2" class="uk-text-muted"></span>
        <p class="uk-margin-small-top"><?php echo $T('COM_RADICALMART_TELEGRAM_REFERRALS_LOGIN_REQUIRED'); ?></p>
    </div>

    <?php elseif (!$this->inChain): ?>
    <div class="uk-card uk-card-default uk-card-body uk-text-center">
        <span uk-icon="icon: users; ratio: 2" class="uk-text-muted"></span>
        <h3 class="uk-margin-small-top"><?php echo $T('COM_RADICALMART_TELEGRAM_REFERRALS_NOT_IN_PROGRAM'); ?></h3>
        <p class="uk-text-muted"><?php echo $T('COM_RADICALMART_TELEGRAM_REFERRALS_NOT_IN_PROGRAM_DESC'); ?></p>
    </div>
    ...
    <div class="uk-card uk-card-default uk-card-body uk-margin-bottom">
        ...
        <?php else: ?>
            ...
            <div class="referral-code-card">
                ...
                <?php if ($code->enabled): ?>
                <!-- Telegram WebApp ссылка (приоритетная - открывает магазин) -->
                <?php if (!empty($code->telegram_webapp_link)): ?>
                <div class="uk-margin-small-bottom">
                    <label class="uk-form-label uk-text-small uk-text-muted">
                        <span uk-icon="icon: bag; ratio: 0.7"></span> <?php echo Text::_('COM_RADICALMART_TELEGRAM_REFERRALS_WEBAPP_LINK'); ?>
                    </label>
                    <div class="uk-flex uk-flex-between uk-flex-middle">
                        <input type="text" value="<?php echo htmlspecialchars($code->telegram_webapp_link); ?>"
                               class="uk-input uk-form-small uk-width-expand uk-margin-small-right"
                               readonly id="webapp-link-<?php echo $code->id; ?>"
                               style="font-size: 12px; background: var(--tg-theme-bg-color, #fff);">
                        <button type="button" class="copy-btn" onclick="copyWebAppLink(<?php echo $code->id; ?>, this)">
                ...
                <!-- Telegram ссылка (открывает чат с ботом) -->
...
function copyWebAppLink(codeId, btn) {
    var input = document.getElementById('webapp-link-' + codeId);
    if (!input) return;
...
async function refreshCart() {
    try {
        ...
        var chat = new URLSearchParams(location.search).get('chat');
