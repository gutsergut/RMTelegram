...
> **Статус**: В работе
> **Текущий этап**: 3.2 — Создание реферального кода
...
- [ ] Расширить `plg_radicalmart_message_telegram` или создать свой обработчик
- [ ] Слушать событие `onRadicalMartAfterChangeOrderStatus`
- [ ] При начислении баллов (статус "Выполнен") отправить сообщение:
  - "🎁 Вам начислено X баллов за заказ №Y"
  - "Ваш баланс: Z баллов (= N ₽)"

#### 4.2 Уведомление о реферальном бонусе
- [ ] При начислении реферальных баллов отправить сообщение родителю:
  - "👥 Ваш реферал совершил заказ! Вам начислено X баллов"

#### 4.3 Уведомление об истечении баллов
- [ ] Scheduled task для проверки сгорающих баллов
- [ ] За N дней до сгорания отправить напоминание
...
- [ ] В карточке товара показать "Кэшбэк: X баллов"
- [ ] В корзине показать "За этот заказ вы получите X баллов"
- [ ] Формула через `PointsHelper::calculatePoints()`

#### 5.2 Виджет баланса в header
- [ ] Показать баланс баллов в навигации WebApp
- [ ] Клик → переход на страницу баллов

#### 5.3 Шаринг реферальной ссылки
- [ ] Кнопка "Поделиться" → Telegram native share
- [ ] Готовый текст с реферальной ссылкой
- [ ] Использовать `Telegram.WebApp.openTelegramLink()` для inline share
